...
export const POST = async (req: NextRequest) => {
    ...
    const pinecone = await getPineconeClient();
    const pineconeIndex = pinecone.index('quill'); // Ensure this is your actual index name

    // Create the PineconeStore with the index
    const vectorStore = await PineconeStore.fromExistingIndex(embeddings, {
        pineconeIndex, // Pass the pineconeIndex here
        namespace: file.id,
    });
